...
def chat_page():
    ...
    # PDF uploader - Multi-file support
    ...
    # PDF selector dropdown if multiple files uploaded
    ...
    # Summarize button (only shown when PDF is loaded)
    if st.session_state.get("pdf_hash"):
        col1, col2 = st.columns([1, 1])
        with col1:
            if st.button("📝 Summarize PDF", key="summarize_btn", help="Generate a summary of the uploaded PDF"):
                with st.spinner("Generating summary..."):
                    ...
                    if summary:
                        ...
                        st.rerun()
        with col2:
            if st.button("💾 Export Chat", key="export_btn", help="Export conversation as PDF", 
...
def export_chat_to_pdf():
    ...
    y_position = height - margin - 30  # Start below title
    
    for msg in st.session_state.messages:
        if y_position < margin + 100:  # New page if we're near bottom
            c.showPage()
        ...
        # Split content into lines manually
        content = msg["content"]
        lines = []
        words = content.split()
        current_line = ""
        ...
        # Calculate space used (original line + wrapped lines)
        lines_used = len(lines) + content.count('\n')  # Account for explicit newlines
        y_position -= (line_height * lines_used + 20)  # Add extra spacing between messages
...
def generate_pdf_summary():
    ...
    all_chunks = vs.similarity_search(" ", k=100)  # Get all chunks by searching for space
    
    # Combine chunks with some overlap
...
def process_uploaded_file(uploaded_file):
    ...
    if pdf_hash not in st.session_state["vector_cache"]:
        with st.spinner("Processing PDF..."):
            ...
            vs = create_vector_store(docs)
            st.session_state["vector_cache"][pdf_hash] = vs
            st.toast("✅ PDF Uploaded & Vectorized", icon="✅")
    else:
        st.toast("📂 PDF already loaded from cache", icon="🧠")
